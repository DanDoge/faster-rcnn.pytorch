        ...
        for img_path in image_ids:
            ...
            for obj in anno_file["record"]["objects"][0][0][0]:
                ...
                if split == "train":
                    ...
                    objs[img_path] = np.append(objs[img_path], {"name":label, "viewpoint":viewpoint, "elevation":elevation, "bbox":bbox})
                    #objs[label] = np.append(objs[label], {"img_path":img_path.split(".")[0], "viewpoint":viewpoint, "bbox":bbox})
...
def gen_annots():
    objs = {}
    for label in os.listdir("./data/pascal3d+1.1/Annotations"):
        for img_path in os.listdir("./data/pascal3d+1.1/Annotations/" + label):
            img_path = img_path.split(".")[0]
            anno_file = loadmat("./data/pascal3d+1.1/Annotations/" + label + "/" + img_path + ".mat")
            for obj in anno_file["record"]["objects"][0][0][0]:
                try:
                ...
                if img_path not in objs:
                    objs[img_path] = []
                objs[img_path] = np.append(objs[img_path], {"name":label, "viewpoint":viewpoint, "elevation":elevation, "bbox":bbox, "difficult":0})
    import pickle
    pickle.dump(objs, open("annots_pascal3d", "wb"), protocol=2)
