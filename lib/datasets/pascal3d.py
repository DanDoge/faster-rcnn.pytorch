...
class pascal3d(imdb):
  def __init__(self, version, image_set):
    imdb.__init__(self, 'pascal_' + version + '_' + image_set)
    self._image_set = image_set

    self._data_path = os.path.join(cfg.DATA_DIR, 'pascal3d+')
    self._classes = ('__background__',  # always index 0
                     'aeroplane_pascal', 'bicycle_pascal', 'boat_pascal',
                     'bottle_pascal', 'bus_pascal', 'car_pascal', 'chair_pascal',
                     'diningtable_pascal',
                     'motorbike_pascal',
                     'sofa_pascal', 'train_pascal', 'tvmonitor_pascal')
    self._class_to_ind = dict(list(zip(self.classes, list(range(self.num_classes)))))
    self._image_ext = '.jpg'
    self.objs = pickle.load(open("objs.pkl", "rb"))
  ...
  def _get_default_path(self):
    ...
    """
    return os.path.join(cfg.DATA_DIR, 'pascal3d+')
