...
class real_image(imdb):
  def __init__(self, version, image_set):
    imdb.__init__(self, 'real_image')
    self._image_set = image_set

    self._data_path = os.path.join(cfg.DATA_DIR, 'real_image')
    ...
    self._image_ext = '.jpg'
    self.objs = pickle.load(open("objs_realimages.pkl", "rb"))
  ...
  def image_path_from_index(self, index):
    ...
    """
    image_path = os.path.join(self._data_path, self.objs[index][0]['label'],
                              index)
  ...
  def _get_default_path(self):
    ...
    """
    return os.path.join(cfg.DATA_DIR, 'real_image')
