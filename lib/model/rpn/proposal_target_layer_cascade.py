...
class _ProposalTargetLayer(nn.Module):
    ...
    def forward(self, all_rois, gt_boxes, num_boxes):
        ...
        labels, viewpoints, elevation, rois, bbox_targets, bbox_inside_weights = self._sample_rois_pytorch(
            all_rois, gt_boxes, fg_rois_per_image,
            rois_per_image, self._num_classes)

        bbox_outside_weights = (bbox_inside_weights > 0).float()

        return rois, labels, viewpoints, elevation, bbox_targets, bbox_inside_weights, bbox_outside_weights
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
        ...
        elevation = gt_boxes[:,:,6].contiguous().view(-1).index((offset.view(-1),)).view(batch_size, -1)

        labels_batch = labels.new(batch_size, rois_per_image).zero_()
        viewpoints_batch = viewpoints.new(batch_size, rois_per_image).zero_()
        elevation_batch = elevation.new(batch_size, rois_per_image).zero_()
        ...
        for i in range(batch_size):
            ...
            elevation_batch[i].copy_(elevation[i][keep_inds])

            # Clamp labels for the background RoIs to 0
            if fg_rois_per_this_image < rois_per_image:
                labels_batch[i][fg_rois_per_this_image:] = 0
                viewpoints_batch[i][fg_rois_per_this_image:] = 0
                elevation_batch[i][fg_rois_per_this_image:] = 0
        ...
        return labels_batch, viewpoints_batch, elevation_batch, rois_batch, bbox_targets, bbox_inside_weights
